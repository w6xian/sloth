...
// QUIC 传输的服务端样例（与 examples/tcp 对照看）。
//
// 把它和 examples/tcp/main.go 逐行对比，业务代码一模一样，差异只有两处：
//   1. QUIC 强制 TLS：必须给一份 *tls.Config（这里运行时生成自签证书，
//      真实部署请换成正式证书）——QUIC 的加密由 TLS 1.3 承担，
//      不像 ws 那样可以明文跑；
//   2. Listen(ctx, sloth.QUIC, addr)：底层是 UDP，一个 QUIC 连接上可以开
//      多条流，每条流对上层就是一条独立连接。
//
// 剩下的（codec、dispatch、bucket、per-call 回包分发、连接回调钩子）全部共用：
// 连回调用的还是 option.WithTcpHandleMessage——它描述的是"一条连接的生命周期"，
// 与跑在 TCP 还是 QUIC 上无关。
//
// 运行：
//   go run ./examples/quic          # 服务端，监听 localhost:8992
//   go run ./examples/quic/client   # 客户端，每 2 秒调一次
//
// 注意：QUIC 端口是 UDP，curl / nc 都打不通（连握手的 ALPN 都对不上）。
...
func main() {
	...
	// ② TLS：QUIC 的必填项，不是可选项。
	//    真实部署请换成正式证书，并让客户端正常校验（别用自签 + 跳过校验）。
	...
	drpc := sloth.ServerConn(server,
		...
		sloth.WithDebugAddr("127.0.0.1:6062"))
	...
	// ⑤ 监听：只改 network 字符串即可换传输（sloth.QUIC == "quic"）。
	//    底层仍是 FN 帧（15 字节定长头 + length 字节载荷）分帧：QUIC 的一条流
	//    是字节流，没有消息边界，分帧规则与 TCP 完全相同。
	if err := drpc.Listen(ctx, sloth.QUIC, "localhost:8992",
	...
	sloth.Infow(ctx, "quic server started",
		"quic", "localhost:8992", "debug", "127.0.0.1:6062")
...
// selfSignedTLS 生成一张自签证书，只为了让样例能直接跑起来。
//
// QUIC 没有 TLS 就握不了手，所以样例必须先有证书；
// 而证书文件（cert.pem/key.pem）会带来"先跑个脚本生成"的前置步骤，
// 因此这里运行时生成——代价是客户端必须跳过校验（见 client/main.go）。
//
...
func selfSignedTLS() (*tls.Config, error) {
	...
	// 样例用固定序列号；真实场景请用 crypto/rand 生成，重复序列号会让证书被拒
	tpl := x509.Certificate{
		SerialNumber: big.NewInt(1),
		Subject:      pkix.Name{CommonName: "sloth-quic-example"},
...
// Handler 连接生命周期回调：QUIC 与 TCP 用的是同一套钩子（无 *http.Request）。
