...
function IsFn(b) {
    ...
    const length = binary.BigEndian.Uint32(arr.slice(11, 15));
...
const _HAS_BIGINT = typeof BigInt !== 'undefined';

/**
 * 以大端序写入 uint64 到 DataView
 * @private
 */
function writeUint64BE(view, offset, value) {
    ...
    // Number 降级: 仅能精确表达 2^53 以内，高 32 位 / 低 32 位处理
...
function readUint64BE(view, offset) {
    ...
    if (hi > 0x1FFFFF) {
        // 超过 Number 安全整数范围，警告但仍返回
        console.warn('[fn.js] uint64 value exceeds Number.MAX_SAFE_INTEGER, precision may be lost. Consider using a BigInt-enabled environment.');
    }
    return combined;
}
